    ...
    if (state) {
      try {
        ...
        if (payload && payload.manual === false && payload.uri) {
          ...
          const isAllowed =
            allowedHostnames.includes(redirectUrl.hostname) ||
            redirectUrl.hostname.endsWith('.run.app');
          if (!isAllowed) {
            throw new Error(
              `Invalid redirect hostname: ${redirectUrl.hostname}. Must be localhost, 127.0.0.1, or a *.run.app domain.`,
            );
          }
