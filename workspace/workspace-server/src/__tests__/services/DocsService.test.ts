...
describe('DocsService', () => {
  ...
  describe('formatText', () => {
    ...
    it('should apply readable paragraph spacing', async () => {
      mockDocsAPI.documents.batchUpdate.mockResolvedValue({ data: {} });

      await docsService.formatText({
        documentId: 'test-doc-id',
        formats: [{ startIndex: 20, endIndex: 80, style: 'bodySpacing' }],
      });
      ...
      expect(mockDocsAPI.documents.batchUpdate).toHaveBeenCalledWith({
        ...
        requestBody: {
          requests: [
            {
              updateParagraphStyle: {
                range: { startIndex: 20, endIndex: 80, tabId: undefined },
                paragraphStyle: {
                  lineSpacing: 115,
    ...
    it('should apply bullet paragraph formatting', async () => {
      mockDocsAPI.documents.batchUpdate.mockResolvedValue({ data: {} });

      await docsService.formatText({
        documentId: 'test-doc-id',
        formats: [{ startIndex: 20, endIndex: 45, style: 'bullet' }],
      });
          ...
          requests: [
            {
              createParagraphBullets: {
                range: { startIndex: 20, endIndex: 45, tabId: undefined },
