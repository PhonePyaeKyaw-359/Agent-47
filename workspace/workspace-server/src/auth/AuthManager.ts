...
export class AuthManager {
  ...
  public async getAuthenticatedClient(): Promise<Auth.OAuth2Client> {
    ...
    // ── ENV-VAR TOKEN INJECTION (multi-user support) ───────────────
    // When the Python API spawns a per-user MCP process it passes tokens
    // via environment variables so no browser auth is required.
    const envAccessToken = process.env['WORKSPACE_ACCESS_TOKEN'];
    const envRefreshToken = process.env['WORKSPACE_REFRESH_TOKEN'] || '';
    if (envAccessToken) {
      logToFile('Using tokens from environment variables (multi-user mode)');

      const envClient = new google.auth.OAuth2({ clientId: CLIENT_ID });
      const envTokens: Auth.Credentials = {
        access_token: envAccessToken,
        refresh_token: envRefreshToken,
        expiry_date: parseInt(
          process.env['WORKSPACE_TOKEN_EXPIRY'] || '0',
          10,
        ),
        scope: process.env['WORKSPACE_TOKEN_SCOPE'] || this.scopes.join(' '),
        token_type: 'Bearer',
      };
      envClient.setCredentials(envTokens);

      // Persist so subsequent calls / refreshes use stored copy
      envClient.on('tokens', async (tokens) => {
        logToFile('Tokens refreshed event received (env-var path)');
        try {
          const current =
            (await OAuthCredentialStorage.loadCredentials()) || {};
          const merged = {
            ...tokens,
            refresh_token: tokens.refresh_token || current.refresh_token,
          };
          await OAuthCredentialStorage.saveCredentials(merged);
          logToFile('Credentials saved after refresh (env-var path)');
        } catch (e) {
          logToFile(`Error saving refreshed credentials (env-var): ${e}`);
        }
      });

      await OAuthCredentialStorage.saveCredentials(envTokens);
      this.client = envClient;

      // Proactive refresh if already expired
      if (this.isTokenExpiringSoon(envTokens)) {
        logToFile('Env-var token is expired, refreshing proactively...');
        try {
          await this.refreshToken();
          logToFile('Env-var token refreshed successfully');
        } catch (error) {
          logToFile(`Failed to refresh env-var token: ${error}`);
        }
      }

      return this.client;
    }
    // ── END env-var injection ──────────────────────────────────────
