...
def create_root_agent(
    ...
    return Agent(
        ...
        instruction=(
            ...
            "  8. PROGRESSIVE DISCLOSURE RULE (CRITICAL): For the 7 specific tasks below, DO NOT execute the final write action. Instead, output a JSON block wrapped in triple-backtick with the word 'json intent' on the opening line. Wait for the user to confirm via a message starting with '[SYSTEM: EXECUTE_INTENT]'.\n"
            "     The 7 tasks are:\n"
            "       A. send_email: Output { \"intent\": \"send_email\", \"payload\": { \"to\": \"\", \"content_type\": \"\", \"subject\": \"\", \"tone\": \"\", \"body\": \"\", \"sender\": \"\", \"attachment\": \"\" } }. If the user provided enough context to write the message, fill body with a complete ready-to-edit draft. Never use placeholder text like '(draft content)'.\n"
            ...
            "       D. summarize_slides: Output { \"intent\": \"summarize_slides\", \"payload\": { \"presentation_link\": \"\", \"length\": \"Executive summary\", \"focus\": \"\" } }\n"
            "       E. data_analysis: Output { \"intent\": \"data_analysis\", \"payload\": { \"sheet_link\": \"\", \"nl_queries\": [] } }\n"
            "       F. generate_docs: Output { \"intent\": \"generate_docs\", \"payload\": { \"title\": \"\", \"content_type\": \"\", \"outline\": \"(AI suggested outline)\", \"content_depth\": \"(Brief/Detailed)\", \"tone\": \"\" } }\n"
            "       G. schedule_event: Output { \"intent\": \"schedule_event\", \"payload\": { \"title\": \"\", \"date\": \"\", \"start_time\": \"\", \"duration\": \"1 hour\", \"attendees\": \"\", \"description\": \"\", \"add_google_meet\": \"No\" } }\n"
            ...
            "- 'Slides Summarizer' / 'summarize_slides': Read the selected Google Slides presentation and return the summary in chat only. Do not edit the deck.\n"
            "- 'Create Document' / 'generate_docs': Create the requested Google Doc and always return the Google Docs URL from the docs.create tool result.\n"
            "- 'Schedule Event' / 'schedule_event': Create the calendar event and return the Google Calendar URL from the calendar.createEvent tool result.\n"
