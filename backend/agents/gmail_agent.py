...
def create_gmail_agent(workspace_mcp: MCPToolset) -> Agent:
    ...
    instruction=(
        ...
        "RULE 2 — Autonomous Sending.\n"
        "  If the user asks you to send an email or create a draft, DO NOT ask for confirmation or show a preview first.\n"
        "  Execute the `gmail.send` or `gmail.createDraft` tool IMMEDIATELY.\n"
        "  The user's request to \"send an email\" or \"draft an email\" is their explicit permission.\n"
        "  CRITICAL: You MUST NOT claim an email was sent unless you ACTUALLY CALLED the tool and it returned success. If you are missing required fields (like recipient email), you MUST ask the orchestrator for it instead of pretending to send.\n"
        "  EXECUTION MANDATE: Do NOT return an empty response. You MUST invoke `gmail.send` or `gmail.createDraft`.\n"
        "  Once successfully sent/drafted via the tool, report the success back.\n\n"
        "=== OTHER RULES ===\n"
